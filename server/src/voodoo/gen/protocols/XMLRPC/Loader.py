...
import voodoo.gen.protocols.XMLRPC.Address as Address
    ...
    for coordination_configuration in coordinations_configuration.coordinations:
        if len(coordination_configuration.parameters) != 1:
            raise LoaderExceptions.InvalidConfigurationException(
                    "Unexpected number of parameters for XMLRPC coordinations, expected 1 and received %s" % len(coordination_configuration.parameters)
                )
        parameter = coordination_configuration.parameters[0]
        if parameter.name != 'address':
            raise LoaderExceptions.InvalidConfigurationException(
                    "Parameter for XMLRPC coordinations should be address, found %s" % parameter.name
                )
        try:
...
def fill_creation(protocol_configuration, coord_address):
    ...
    if len(creation_configuration.parameters) != 2:
        raise LoaderExceptions.InvalidConfigurationException(
                "Unexpected number of parameters for XMLRPC creation, expected 2 and received %s" % len(creation_configuration.parameters)
            )
    ...
    if creation_configuration.parameters[0].name == 'address':
        ...
        raise LoaderExceptions.InvalidConfigurationException(
                "Parameter 'address' not found in XMLRPC creation"
            )
    if creation_configuration.parameters[0].name == 'port':
        ...
        raise LoaderExceptions.InvalidConfigurationException(
                "Parameter 'port' not found in XMLRPC creation"
            )
    ...
    protocol_configuration.filled_creation = ('XMLRPC',(address,port_number), {})
