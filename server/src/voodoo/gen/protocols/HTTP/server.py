...
_http_server = None
_http_server_functions = []
_http_server_lock = threading.Lock()

class UtilRequestHandlerClass(BaseHTTPServer.BaseHTTPRequestHandler):
...
class _HTTPServerResourceManager(ResourceManager.ResourceManager):
...
class _AvoidTimeoutHTTPServer(SocketServer.ThreadingMixIn, BaseHTTPServer.HTTPServer):
...
def generate(cfg_manager, methods):
    ...
    class ServerHTTP(threading.Thread):
        def __init__(self,who,port):
            super(ServerHTTP,self).__init__()
            self.setName(counter.next_name("ServerHTTP"))
            self._register_http_server(who,port)
        ...
        def _register_http_server(self,who,port):
            _http_server_lock.acquire()
            try:
                global _http_server
                if _http_server == None:
                    _http_server = {}
                if not _http_server.has_key(port):
                    _http_server[port] = _AvoidTimeoutHTTPServer((who,port), requestHandler=UtilRequestHandlerClass, allow_none = True)
                    _http_server[port].socket.settimeout(MAX_TIMEOUT)
                self.server = _http_server[port]

                for i in all_methods:
                    if i in _http_server_functions:
                        log.log(ServerHTTP,log.level.Warning,'Method "%s" already served by server "%s"' % (i,self))
                    ...
                    _http_server_functions.append(i)
    
            finally:
                _http_server_lock.release()
    ...
    for method_name in all_methods:
        ...
        setattr(ServerHTTP, method_name, func)
