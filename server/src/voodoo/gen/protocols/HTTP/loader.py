...
import voodoo.gen.protocols.HTTP.address as Address
    ...
    for coordination_configuration in coordinations_configuration.coordinations:
        if len(coordination_configuration.parameters) != 1:
            raise LoaderErrors.InvalidConfigurationError(
                    "Unexpected number of parameters for HTTP coordinations, expected 1 and received %s" % len(coordination_configuration.parameters)
                )
        parameter = coordination_configuration.parameters[0]
        if parameter.name != 'address':
            raise LoaderErrors.InvalidConfigurationError(
                    "Parameter for HTTP coordinations should be address, found %s" % parameter.name
                )
        try:
            address = Address.Address(parameter.value)
        except Exception as e:
            raise LoaderErrors.InvalidConfigurationError(
                    "Invalid address format for HTTP coordinations: %s" % e
                )
...
def fill_creation(protocol_configuration, coord_address):
    ...
    if len(creation_configuration.parameters) != 2:
        raise LoaderErrors.InvalidConfigurationError(
                "Unexpected number of parameters for HTTP creation, expected 2 and received %s" % len(creation_configuration.parameters)
            )
    ...
    if creation_configuration.parameters[0].name == 'address':
        ...
        raise LoaderErrors.InvalidConfigurationError(
                "Parameter 'address' not found in HTTP creation"
            )
    if creation_configuration.parameters[0].name == 'port':
        ...
        raise LoaderErrors.InvalidConfigurationError(
                "Parameter 'port' not found in HTTP creation"
            )
    ...
    protocol_configuration.filled_creation = ('HTTP',(address,port_number), {})
