...
from sqlalchemy import Column, Boolean, Integer, BigInteger, String, DateTime, Date, Text, ForeignKey, UniqueConstraint, \
    Table, Index, Unicode, UnicodeText
...
class DbInvitation(Base):
    ...
    __table_args__ = (UniqueConstraint('unique_id'), TABLE_KWARGS)
    ...
    def __repr__(self):
        return "DbInvitation(id = %r, unique_id = %r, group_id=%r, creation_date=%r, expire_date=%r, max_number=%r)" % (
...
class DbAcceptedInvitation(Base):
    ...
    __table_args__ = (UniqueConstraint('invitation_id', 'user_id'), TABLE_KWARGS)

    id = Column(Integer, primary_key=True)
    invitation_id = Column(Integer, ForeignKey("Invitation.id"))
    user_id = Column(Integer, ForeignKey("User.id"))
    accepted_date = Column(DateTime, nullable=False)
    registered = Column(Boolean, nullable=False)

    user = relationship("DbUser", backref=backref("accepted_invitations"))
    invitation = relationship("DbInvitation",
                              backref=backref("accepted_invitations", order_by=id, cascade='all,delete'))

    def __init__(self, invitation, user, registered):
        super(DbAcceptedInvitation, self).__init__()
    ...
    def __repr__(self):
        return "DbAcceptedInvitation(id = %r, invitation_id = %r, user_id = %r, accepted_date = %r, registered = %r)" % (
            self.id, self.invitation_id, self.user_id, self.accepted_date, self.registered
        )
...
class DbExperimentInstance(Base):
    ...
    scheduler_resource = relationship("DbSchedulerResource",
                                      backref=backref("experiment_instances", order_by=id, cascade='all,delete'))
    experiment = relationship("DbExperiment",
                              backref=backref("experiment_instances", order_by=id, cascade='all,delete'))
...
class DbSchedulerExternalExperimentEntry(Base):
    ...
    def __unicode__(self):
        return u"Entry for experiment %s on scheduler %s with config = %r" % (
            self.experiment, self.scheduler, self.config)
...
class DbUserUsedExperiment(Base):
    ...
    def __init__(self, user=None, experiment=None, start_date=None, origin=None, coord_address=None,
                 reservation_id=None, end_date=None, max_error_in_millis=None, finish_reason=None,
                 permission_permanent_id=None, group_permission=None, user_permission=None, role_permission=None,
                 session_time_micro=None, hostname=None, city=None, country=None, most_specific_subdivision=None):
        ...
        if end_date is not None:
            self.session_time_micro = (self.end_date - self.start_date).seconds * 1e6 + (
                self.end_date - self.start_date).microseconds
...
class DbUserUsedExperimentPropertyValue(Base):
    ...
    property_name = relationship("DbUserUsedExperimentProperty",
                                 backref=backref("values", order_by=id, cascade='all,delete'))
    experiment_use = relationship("DbUserUsedExperiment",
                                  backref=backref("properties", order_by=id, cascade='all,delete'))
...
class DbUserFile(Base):
    ...
    def __init__(self, experiment_use=None, file_sent=None, file_hash=None, timestamp_before=None, file_info=None,
                 response=None, timestamp_after=None):
...
class DbUserCommand(Base):
    ...
    experiment_use = relationship("DbUserUsedExperiment",
                                  backref=backref("commands", order_by=id, cascade='all,delete'))
