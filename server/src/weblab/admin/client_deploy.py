...
def compile_client(war_location, client_location):
    ...
    if os.path.exists(about_txt):
        ...
        if GWT_VERSION_NUMBER in first_line:
            ...
            print >> sys.stderr, ""
            print >> sys.stderr, "Old GWT version found (required: %s). Trying to compile the client..." % GWT_VERSION_NUMBER
    else:
        print >> sys.stderr, ""
        print >> sys.stderr, "No GWT version found. Trying to compile the client..."
    ...
    if must_download:
        ...
        print >> sys.stderr, "(please wait a few minutes; GWT is ~100 MB)"
        try:
