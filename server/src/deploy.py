...
@deployer(20)
def deploy_database_data(error_handler):
    cwd = os.getcwd()
    os.chdir('../sql')
    pr = subprocess.Popen(sys.executable + " run_tests.py", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    result = pr.wait()
    os.chdir(cwd)
    if result != 0:
...
@deployer(50)
def deploy_coordination_database(error_handler):
    try:
        import weblab.user_processing.coordinator.dao as dao
    except ImportError, ie:
        error_handler.write_msg('Could not import dao package; %s' % ie)
        return True
    try:
        from sqlalchemy import create_engine
    except ImportError, ie:
        error_handler.write_msg('Could not import sqlalchemy: %s' % ie)
        return True

    engine = create_engine('mysql://weblab:weblab@localhost/WebLabCoordination', echo = False)

    dao.WaitingReservation.__table__.drop( bind=engine, checkfirst = True)
    dao.CurrentReservation.__table__.drop( bind=engine, checkfirst = True)
    dao.Reservation.__table__.drop(        bind=engine, checkfirst = True)
    dao.ExperimentInstance.__table__.drop( bind=engine, checkfirst = True)
    dao.ExperimentType.__table__.drop(     bind=engine, checkfirst = True)
