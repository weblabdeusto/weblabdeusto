...
def index(req):
    ...
    try:
        ...
        try:
            SENTENCE = "SELECT u.login, u.full_name, e.name, c.name, uue.start_date, uue.origin " + \
            ...
            result = """<html><head><title>Latest uses</title></head><body><table cellspacing="20">
                        <tr> <td><b>User</b></td> <td><b>Name</b></td> <td><b>Experiment</b></td> <td><b>Date</b></td> <td><b>From </b> </td></tr>
                        """
            for user_login, user_full_name, experiment_name, category_name, start_date, uue_from in elements:
                result += "\t<tr> <td> %s </td> <td> %s </td> <td> %s </td> <td> %s </td> <td> %s </td> </tr>\n" % ( user_login, user_full_name, experiment_name + '@' + category_name, utc2local_str(start_date), uue_from )
        finally: 
            cursor.close()
    finally:
        connection.close()
