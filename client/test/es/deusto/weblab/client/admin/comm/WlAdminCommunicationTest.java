...
public class WlAdminCommunicationTest extends WlCommonCommunicationTest {
	...
	public void testGetExperimentUses() {
		...
		final String SERIALIZED_MESSAGE = "serialized get experiment uses request";
		final String ERROR_MESSAGE = "whatever the error message";
		
		final Date fromDate = new Date();
		final Date toDate = new Date();
		final User user = new User("jaime.irurzun", "Jaime Irurzun", "jaime.irurzun@opendeusto.es", new Role("student"));
		final Experiment experiment = new Experiment(0, "ud-fpga", new Category("FPGA experiments"), new Date(), new Date());
		final String origin = "unknown";
		
		final ArrayList<ExperimentUse> expectedExperimentUses = new ArrayList<ExperimentUse>();
		expectedExperimentUses.add(new ExperimentUse(fromDate, toDate, user, experiment, origin));
		
		weblabSerializer.appendReturn(
					FakeWlAdminSerializer.PARSE_GET_EXPERIMENT_USES_RESPONSE, 
					expectedExperimentUses
				);
		weblabSerializer.appendReturn(
					FakeWlAdminSerializer.SERIALIZE_GET_EXPERIMENT_USES_REQUEST, 
					SERIALIZED_MESSAGE
				);
		
		IExperimentUsesCallback callback = new IExperimentUsesCallback(){
			@Override
			public void onSuccess(ArrayList<ExperimentUse> experimentUses) {
				Assert.assertEquals(expectedExperimentUses.size(), experimentUses.size());
				Assert.assertEquals(expectedExperimentUses.get(0), experimentUses.get(0));
				WlAdminCommunicationTest.this.stepCounter++;
			}
		...
		requestBuilder.setNextReceivedMessage(SERIALIZED_MESSAGE);		
		comms.getExperimentUses(sessionId, null, null, -1, -1, callback);
		Assert.assertEquals(1, this.stepCounter);
		
		requestBuilder.setNextToThrow(new RequestException(ERROR_MESSAGE));
		callback = new IExperimentUsesCallback(){
			@Override
			public void onSuccess(ArrayList<ExperimentUse> experimentUses) {
				Assert.fail("onSuccess not expected");
			}
